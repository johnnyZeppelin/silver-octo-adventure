...
def data_test_choose(test_case, data_path):
    test_case_list = list(range((test_case - 1) * 32 + 1,
                                test_case * 32 + 1))  # 32 is the number of sample in each distorted image group
    ...
    for i, name in enumerate(test_case_list):
        if test_case_list[i] in range(10):
            ...
            test_group.append(test_name)
            # print(test_name)
    # print('test_group: ', len(test_group))
    return test_group
...
def data_train_choose(train_case, data_path):
    ...
    for i, name in enumerate(train_cases_list):
        if train_cases_list[i] in range(10):
            ...
            train_group.append(train_name)
            # print(test_name)
    # print('train_group', len(train_group))
    return train_group


# get the affinity matrix
def get_aff_path():
    set_root = 'Database/attention_weightmatrix/A_binary_quandratic/A_01_e.pt'
    print(set_root)
    return set_root


# /content/drive/MyDrive/Machine Learning/ha_/Nud_GCN/VGCN_PyTorch/Database/affinity_matrix/Affinity_modified/A_Gaussian_0.4.pt
...
# # get the error map
# def get_errmapspath(is_training):
#     sets_root = '/content/drive/MyDrive/Machine Learning/ha_/Nud_GCN/VGCN_PyTorch/Database/'
#     if is_training:
#         sets = [
#             'error_map/data_full'
#         ]
...
# 'error_map/2_Group_test/case_2/train'
# Retangular_Nud
...
def get_labelspath(is_training):
    sets_root = '/content/drive/MyDrive/MachineLearning/ha_/Nud_GCN/VGCN_PyTorch/Database/'
    if is_training:
        sets = [
            'nud_fovall_label'
        ]
...
def get_data_path(data_path, train_case, test_case):
    ...
    # print('test_set_list', len(test_set_list))
    # print(test_set_list)
    # print('train_set_list', len(train_set_list))
